...
<figcaption class="fig-caption-margin">${captionHtml}</figcaption>
	...
	eleventyConfig.addPairedShortcode("mn", function (content, anchor) {
		...
		if (anchor) {
			...
			return `<span class="mn-ref" data-mn-id="${noteId}"><span class="mn-anchor-text" aria-describedby="${noteId}">${anchor}<button type="button" class="mn-marker" aria-expanded="false" aria-controls="${noteId}" aria-label="Show margin note ${counters.mn}: ${anchorText}">${counters.mn}</button></span><span class="mn-note" id="${noteId}" role="note" aria-label="Margin note ${counters.mn}"><span class="mn-note-number" aria-hidden="true">${counters.mn}.</span>${noteHtml}</span></span>`;
		} else {
